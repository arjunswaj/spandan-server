...
import com.asb.gae.spandan.Constants;
import com.asb.gae.spandan.Message;
import com.asb.gae.spandan.MulticastResult;
import com.asb.gae.spandan.Result;
import com.asb.gae.spandan.Sender;
...
public class SendMessageServlet extends BaseServlet {
  ...
  static final String PARAMETER_MESSAGE = "gcmMessage";
  static final String PARAMETER_SENDER = "gcmSender";
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    String gcmMessage = req.getParameter(PARAMETER_MESSAGE);
    String gcmSender = req.getParameter(PARAMETER_SENDER);
    if (regId != null) {
      sendSingleMessage(regId, resp, gcmMessage, gcmSender);
      return;
    }
    String multicastKey = req.getParameter(PARAMETER_MULTICAST);
    if (multicastKey != null) {
      sendMulticastMessage(multicastKey, resp, gcmMessage, gcmSender);
      return;
    }
  ...
  private Message createMessage(String gcmMessage, String gcmSender) {
    Message message = new Message.Builder().addData("Update", gcmMessage)
        .addData("Sender", gcmSender).build();
    return message;
  }

  private void sendSingleMessage(String regId, HttpServletResponse resp,
      String gcmMessage, String gcmSender) {
    logger.info("Sending message to device " + regId);
    Message message = createMessage(gcmMessage, gcmSender);
    Result result;
    try {
    ...
    if (result.getMessageId() != null) {
      ...
      if (error.equals(Constants.ERROR_NOT_REGISTERED)) {
        ...
        logger
            .severe("Error sending message to device " + regId + ": " + error);
      }
    }
  }

  private void sendMulticastMessage(String multicastKey,
      HttpServletResponse resp, String gcmMessage, String gcmSender) {
